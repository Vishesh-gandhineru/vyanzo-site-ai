...
const services = [
  {
    ...
    description: "Vyanzo provides a high-level engineering resource to help clients design or optimize products.",
    highlights: [
      { label: "Expertise", value: "Access to a team of over 10 engineers." },
      { label: "Experience", value: "A combined 150 years of experience in the casting industry." },
      { label: "Focus", value: "Designing and optimizing products to specifically suit customer needs and technical requirements." }
    ]
  },
  {
    id: "production",
    ...
    description: "Vyanzo manages the entire manufacturing lifecycle, ensuring quality at every stage according to specific client requirements.",
    highlights: [
  ...
  },
  {
    ...
    description: "Vyanzo offers specialized assistance with international certification processes to ensure products meet global safety and quality standards.",
    highlights: [
      { label: "Available Certifications", value: "EN124-2 (Ductile iron manhole covers)" },
      { label: "COPRO", value: "Belgian certification" },
      { label: "FM / UL", value: "Fire protection/safety" },
      { label: "Other", value: "BS-Kitemark / KOMO / ICMQ / MPA" },
      { label: "Compliance", value: "RoHS compliance" }
    ]
  }
];
              ...
              {services.map((service) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`relative z-10 flex items-center gap-4 w-full ${isActive ? "text-white" : ""}`}>
                      <div className={`shrink-0 p-3 rounded-xl transition-colors ${
                        isActive ? "bg-brand-primary/20 text-brand-primary" : "bg-white text-brand-ash group-hover:text-brand-primary border border-brand-ash/20"
                      }`}>
                        {service.icon}
                      </div>
                ...
                {services.map((service) => (
                  service.id === activeTab && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -20, filter: "blur(10px)", display: "none" }}
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-auto">
                        {service.highlights.map((highlight, idx) => (
                          <div key={idx} className="bg-white p-6 rounded-2xl shadow-sm border border-brand-ash/10 hover:-translate-y-1 transition-transform">
                            <div className="flex items-start gap-4">
                              <CheckCircle2 className="w-5 h-5 text-brand-primary shrink-0 mt-0.5" />
                              <div>
                                <h4 className="font-bold text-bg-dark mb-1">{highlight.label}</h4>
                                <p className="text-text-light-bg/70 text-sm leading-relaxed">{highlight.value}</p>
                              </div>
                            </div>
                          </div>
                        ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
               {productCategories.map((cat, idx) => (
                 <motion.div 
                   ...
                   className="bg-white rounded-3xl p-8 flex flex-col items-center justify-center text-center gap-6 shadow-sm border border-brand-ash/10 hover:border-brand-primary/30 hover:shadow-[0_10px_40px_rgba(253,231,76,0.15)] hover:-translate-y-2 transition-all duration-300 group cursor-pointer"
                 >
