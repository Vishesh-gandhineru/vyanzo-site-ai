...
function FilterCheckbox({
  ...
  return (
    ...
    >
      ...
      <span className="text-text-light-bg/80 font-sans font-normal text-sm">
        {label}
      </span>
    </label>
  );
...
export default function ProductGrid() {
  ...
  const filtered = products
    ...
    .filter(
      (p) =>
        selectedCertTypes.length === 0 ||
        selectedCertTypes.includes(p.certificationType),
    )
    .filter(
      (p) =>
        selectedGeos.length === 0 ||
              ...
              {filtered.map((product) => (
                ...
                >
                  ...
                  <div className="w-full h-[240px] relative overflow-hidden bg-brand-ash/10">
                    ...
                    <div
                      className={`absolute top-3 right-3 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-sans font-bold tracking-wider uppercase ${certChipClass(product.certificationType)}`}
                    >
                    ...
                    <div className="flex gap-2 mb-5 flex-wrap">
                      {product.specificationFiles.length > 0 && (
                        <span className="text-[10px] font-bold tracking-wide uppercase px-2.5 py-1 rounded-full bg-blue-50 text-blue-600 border border-blue-100">
                          {product.specificationFiles.length} Spec
                          {product.specificationFiles.length > 1 ? "s" : ""}
                        </span>
                      )}
                      {product.certificationFiles.length > 0 && (
                        <span className="text-[10px] font-bold tracking-wide uppercase px-2.5 py-1 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-100">
                          {product.certificationFiles.length} Cert
                          {product.certificationFiles.length > 1 ? "s" : ""}
                        </span>
                      )}
                    </div>
